...
def process_all_model(modelname, dates, topics, y_datafile, stocks, test_date):
    ...
    if test_date:
        y_dic_test, x_dic_test, x3_dic_test = modelling_part1.collect_all_x_and_y(test_date, topics, y_datafile, stocks)
    else:
        testing_data = None
...
def select_on_xs(selection_on_x, x1_x2, x3):
    ...
    print(f"need transpose, {training_x}")
    ...
    def model_building(self, model_name, test):
        ...
        if model_name == "linear":
            ...
            if model_name == "logistic":
                print(f"Xs is {Xs}, y_dummy is {y_dummy}")
            ...
            print("y_dummy is not varying so only linear regression model is provided")
            model = None
        return model

...
def process_all_model(modelname = MODELNAME, dates = TRAINING_DATES, topics = TOPIC, y_datafile = Y_FILENAME, stocks = STOCK, test_date = TESTING_DATES):
    """
    Input:
        topic: list
        modelname: list
    ...
    y_dic, x_dic, x3_dic = modeling_preparing_data.collect_all_x_and_y(dates, topics, y_datafile, stocks)

    print("++++++++++++++++++ starting to prepare testing data +++++++++++++++++++")
    if test_date:
        y_test, x_dic_test, x3_dic_test = modeling_preparing_data.collect_all_x_and_y(test_date, topics, y_datafile, stocks)
    else:
        testing_data = None
    ...
    for topic_name in topics:
        for stock_name in stocks:
            for name in modelname:
                ...
                if test_date:
                    training_x_test = select_on_xs(selection_on_x, x_dic_test, x3_dic_test)
